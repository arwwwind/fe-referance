      ...
      <Row gutter={15} type="flex">
        {checkValidValue(props.data.daySinceLastReferral) ? (
          <Col className="stats-item" sm={6} xs={8}>
            <div className="stats-value">{formatMetricsValue(props.data.daySinceLastReferral)}</div>
            <div className="stats-name">Day Since Last Referral</div>
          </Col>
        ) : null}
        {checkValidValue(props.data.totalReferrals) ? (
          <Col className="stats-item" sm={6} xs={8}>
            <div className="stats-value">{formatMetricsValue(props.data.totalReferrals)}</div>
            <div className="stats-name">Total Referrals</div>
          </Col>
        ) : null}
        {checkValidValue(props.data.openCases) ? (
          <Col className="stats-item" sm={6} xs={8}>
            <div className="stats-value">{formatMetricsValue(props.data.openCases)}</div>
        ...
        {checkValidValue(props.data.iwoSuccessRate) ? (
          <Col className="stats-item" sm={6} xs={8}>
            <div className="stats-value">{props.data.iwoSuccessRate !== '-' ? `${formatMetricsValue(props.data.iwoSuccessRate)}%` : '-'}</div>
